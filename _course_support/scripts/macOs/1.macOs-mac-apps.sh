...
#-------------------------------------------------
# Install Zsh (set as default, Intel & Silicon)
#-------------------------------------------------
echo "$scriptTitle Ensuring Zsh is the default shell..."

# Choose the best Zsh path (prefer Homebrew if present)
if [ -x /opt/homebrew/bin/zsh ]; then
    ...
    abort "Zsh is not installed. Please install Zsh and re-run."
fi

echo "Using Zsh at: $ZSH_PATH"

# Ensure the chosen Zsh is in /etc/shells (required by chsh)
if ! grep -qx "$ZSH_PATH" /etc/shells; then
    echo "Adding $ZSH_PATH to /etc/shells (requires sudo)..."
    echo "$ZSH_PATH" | sudo tee -a /etc/shells >/dev/null || abort "Failed to add Zsh to /etc/shells."
fi

# Read current login shell (do not rely on $SHELL inside scripts)
CURRENT_SHELL="$(dscl . -read /Users/$USER UserShell 2>/dev/null | awk '{print $2}')"
if [ "$CURRENT_SHELL" = "$ZSH_PATH" ]; then
    echo "Zsh is already the default shell: $CURRENT_SHELL"
else
    echo "Changing default shell for $USER to $ZSH_PATH ..."
    chsh -s "$ZSH_PATH" "$USER" || abort "Failed to set Zsh as the default shell."
fi
